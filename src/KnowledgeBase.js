...
import { 
    ...
    FaListUl, FaListOl, FaQuoteLeft, FaAlignLeft, FaAlignCenter, 
    FaAlignRight, FaUndo, FaRedo, FaSave, FaTimes, FaPen , FaStickyNote
} from 'react-icons/fa';
  ...
  return (
    <div className="kb-menu-bar">
        <div className="menu-group">
            ...
            <button type="button" onClick={() => editor.chain().focus().toggleUnderline().run()} className={editor.isActive('underline') ? 'is-active' : ''}><FaUnderline /></button>
        ...
        <div className="menu-group">
            <button type="button" onClick={() => editor.chain().focus().setTextAlign('left').run()} className={editor.isActive({ textAlign: 'left' }) ? 'is-active' : ''}><FaAlignLeft /></button>
            <button type="button" onClick={() => editor.chain().focus().setTextAlign('center').run()} className={editor.isActive({ textAlign: 'center' }) ? 'is-active' : ''}><FaAlignCenter /></button>
            <button type="button" onClick={() => editor.chain().focus().setTextAlign('right').run()} className={editor.isActive({ textAlign: 'right' }) ? 'is-active' : ''}><FaAlignRight /></button>
        </div>
        <div className="menu-group">
...
const KnowledgeBase = ({ onSave }) => {
    ...
    const editor = useEditor({
        extensions: [
            ...
            Placeholder.configure({ placeholder: 'Start writing your sacred notes and mantras...' })
        ],
        content: '',
        editable: false,
    });
        ...
        <div className="kb-view-container">
            <div className="kb-view-header">
                ...
                <div className="kb-view-actions">
                    {isEditMode ? (
                        <>
                            <button onClick={() => setIsEditMode(false)} className="kb-btn secondary">
                                <FaTimes /> Cancel
                            </button>
                        ...
                        <button onClick={() => setIsEditMode(true)} className="kb-btn primary">
                            <FaPen /> Edit Mantras
                        </button>
                    )}
                </div>
