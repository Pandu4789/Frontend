        ...
        <main className="pg-main-content">
          {selectedPooja ? (
            <div className="pg-card animate-fade-in">
              <div className="pg-card-header">
                ...
                <div className="pg-meta-tags">
                  <span className="pg-tag"><FaTag /> {selectedPooja.category || 'General'}</span>
                  <span className="pg-tag"><FaClock /> {selectedPooja.duration || 'Flexible'}</span>
                </div>
              </div>

              <div className="pg-description-box">
                <p>{selectedPooja.description || 'No description available for this ritual.'}</p>
              </div>
                  ...
                  {items.length > 0 ? (
                    items.map(item => (
                      <div key={item.id} className="pg-item-row">
                        ...
                        <span className="pg-item-qty">{item.quantity} {item.unit || ''}</span>
                      </div>
                    ))
                  ) : (
                    <div className="pg-empty-state">
                      <FaInfoCircle /> <p>Select a ritual or check back later.</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="pg-card-footer">
                <div className="pg-price-info">
                  ...
                  <span className="value">{selectedPooja.estimatedPrice || 'Contact Priest'}</span>
                </div>
            ...
            <div className="pg-welcome-state">
              <div className="pg-welcome-icon"><FaLeaf /></div>
              <h2>Ready to Begin?</h2>
              <p>Choose a pooja ritual from the sidebar to view the checklist.</p>
            </div>
          )}
        </main>
      </div>
