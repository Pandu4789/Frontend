...
const ManagePoojaServices = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(
          `${API_BASE}/api/events/${currentService.id}`,
          currentService,
        );
    ...
    <>
      ...
      <div className="manage-services-container">
        ...
        <div className="service-list-grid">
          {isLoading ? (
            <p className="loading-error-message">Loading services...</p>
          ) : (
            services.map((event) => (
              <div className="service-card" key={event.id}>
                ...
                <div className="service-card-body">
                  <div className="service-detail-item">
                    <FaTag className="icon" /> <strong>Category:</strong>{" "}
                    {event.category || "N/A"}
                  </div>
                  <div className="service-detail-item">
                    <FaClock className="icon" /> <strong>Duration:</strong>{" "}
                    {event.duration || "N/A"}
                  </div>
                  <div className="service-detail-item">
                    <FaDollarSign className="icon" /> <strong>Price:</strong>{" "}
                    {event.estimatedPrice || "N/A"}
                  </div>
              ...
              <form onSubmit={handleSubmit} className="admin-modal-body">
                <div className="form-grid">
                  <input
                    ...
                    placeholder="Service Name (e.g., Griha Pravesh)"
                    required
                  />
                  <input
                    ...
                    onChange={handleInputChange}
                    placeholder="Category (e.g., Lifecycle Events)"
                  />
                  <input
                    className="form-input"
                    name="duration"
                  ...
                  <input
                    className="form-input col-span-2"
                    name="estimatedPrice"
                    value={currentService.estimatedPrice}
                    onChange={handleInputChange}
                    placeholder="Estimated Price (e.g., $351)"
                  />
                  <textarea
                    className="form-input col-span-2"
                    name="description"
