...
const MuhurtamRequests = () => {
  ...
  const counts = useMemo(
    () => ({
      rituals: {
        ...
        pending: appointmentBookings.filter(
          (b) =>
            (b.status || "PENDING").toUpperCase() === "PENDING" &&
            !isExpired(b.date),
        ).length,
        accepted: appointmentBookings.filter(
          (b) => b.status?.toUpperCase() === "ACCEPTED",
        ).length,
        rejected: appointmentBookings.filter(
  ...
  const handleAction = async (id, action) => {
    try {
      if (activeTab === "requests") {
        await axios.put(`${API_BASE}/api/muhurtam/${id}/viewed`, {
          viewed: true,
        });
      } else {
  ...
  const filteredData = useMemo(() => {
    ...
    return rawData.filter((item) => {
      const matchesSearch = (item.name + (item.eventName || ""))
        .toLowerCase()
      ...
      if (activeTab === "bookings") {
        ...
        if (statusFilter === "PENDING")
          return status === "PENDING" && !isExpired(item.date);
              ...
              <section className="yb-drawer-section">
                ...
                <div className="yb-drawer-info">
                  <p>
                    ...
                    {selectedItem.eventName || "Analysis"}
                  </p>
                  {activeTab === "requests" ? (
                    ...
                    <>
                      <p>
                        <strong>Scheduled:</strong> {selectedItem.date} @{" "}
                        {selectedItem.start}
                      </p>
                    </>
                  )}
            ...
            <div className="yb-drawer-footer">
              {getStatusMeta(selectedItem).cls === "pending" ? (
                <div className="yb-action-group">
                  <button
                    className="yb-btn-reject-full"
                    onClick={() => handleAction(selectedItem.id, "reject")}
                  >
                    Decline
                  </button>
                  <button
                    className="yb-btn-accept-full"
                    onClick={() =>
                      handleAction(
                        selectedItem.id,
                        activeTab === "requests" ? "view" : "accept",
                      )
                    }
                  >
                    {activeTab === "requests"
