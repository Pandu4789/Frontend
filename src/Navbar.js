...
const Navbar = ({ onLogout }) => {
    ...
    const fetchCounts = async () => {
      try {
        ...
        setStats({
          rituals: {
            all: rituals.length,
            pending: rituals.filter(
              (b) => (b.status || "PENDING").toUpperCase() === "PENDING",
            ).length,
            accepted: rituals.filter(
              (b) =>
                b.status?.toUpperCase() === "ACCEPTED" ||
                b.status?.toUpperCase() === "CONFIRMED",
            ).length,
            rejected: rituals.filter(
          ...
          schedule: {
            today: rituals.filter(
              (b) =>
                b.date &&
                isToday(parseISO(b.date.split("T")[0])) &&
                b.status?.toUpperCase() !== "REJECTED",
            ).length,
            upcoming: rituals.filter(
              (b) =>
                b.date &&
                isAfter(parseISO(b.date.split("T")[0]), today) &&
                (b.status?.toUpperCase() === "ACCEPTED" ||
                  b.status?.toUpperCase() === "CONFIRMED"),
            ).length,
          },
  ...
  return (
    <>
              ...
              <div className="profile-dropdown" ref={dropdownRef}>
                ...
                <div className="dropdown-menu">
                  ...
                  <div
                    ...
                    className="dropdown-item"
                  >
                    <FaLock className="dropdown-icon" /> Password
                  </div>
                  <div
      ...
      <aside
        ref={sidebarRef}
        className={`priestify-sidebar-v2 ${isSidebarOpen ? "open" : "closed"}`}
      >
        <div className="sidebar-v2-inner">
          <div className="sidebar-v2-menu">
            <div
              onClick={() => goToPage("/Dashboard")}
              className={`v2-nav-item ${location.pathname === "/Dashboard" ? "active" : ""}`}
            >
              <span className="v2-icon">
                <FaHome />
              </span>
              <span className="v2-text">Dashboard</span>
            </div>
            {/* EARNINGS MOVED BELOW DASHBOARD */}
            <div
              onClick={() => goToPage("/Earnings")}
              className={`v2-nav-item ${location.pathname === "/Earnings" ? "active" : ""}`}
            >
              <span className="v2-icon">
                <FaChartBar />
              </span>
              <span className="v2-text">Earnings</span>
            </div>
            {/* MUHURTAM FINDER RESTORED */}
            <div
              onClick={() => goToPage("/Mohurtam")}
              className={`v2-nav-item ${location.pathname === "/Mohurtam" ? "active" : ""}`}
            >
              <span className="v2-icon">
                <FaPray />
              </span>
              <span className="v2-text">Muhurtam Finder</span>
            </div>
          </div>
          ...
          >
            INBOX / REQUESTS{" "}
            {requestsExpanded ? <FaChevronUp /> : <FaChevronDown />}
          </div>
          {requestsExpanded && (
            <div className="sidebar-v2-nested">
              <div className="nested-group">
                <div className="nested-title">
                  <FaClipboardList /> Muhurtam Inquiries
                </div>
                <div
                  className="nested-item"
                  onClick={() => goToPage("/Requests", "ALL", "requests")}
                >
                  Total{" "}
                  <span className="v2-badge blue">{stats.muhurtams.all}</span>
                </div>
                <div
                  className="nested-item"
                  onClick={() => goToPage("/Requests", "NEW", "requests")}
                >
                  New Inquiry{" "}
                  <span className="v2-badge gold">{stats.muhurtams.new}</span>
                </div>
              </div>

              <div className="nested-group">
                <div className="nested-title">
                  <FaCalendarCheck /> Ritual Bookings
                </div>
                <div
                  className="nested-item"
                  onClick={() => goToPage("/Requests", "PENDING", "bookings")}
                >
                  Pending{" "}
                  <span className="v2-badge gold">{stats.rituals.pending}</span>
                </div>
                <div
                  className="nested-item"
                  onClick={() => goToPage("/Requests", "ACCEPTED", "bookings")}
                >
                  Accepted{" "}
                  <span className="v2-badge green">
                    {stats.rituals.accepted}
                  </span>
                </div>
                <div
                  className="nested-item"
                  onClick={() => goToPage("/Requests", "REJECTED", "bookings")}
                >
                  Rejected{" "}
                  <span className="v2-badge red">{stats.rituals.rejected}</span>
                </div>
              </div>
            </div>
          )}
          ...
          {bookingsExpanded && (
            <div className="sidebar-v2-nested">
              <div
                className="nested-item"
                onClick={() =>
                  goToPage("/Dashboard", "ALL", "bookings", "today")
                }
              >
                Today's Rituals{" "}
                <span className="v2-badge blue">{stats.schedule.today}</span>
              </div>
              <div
                className="nested-item"
                onClick={() =>
                  goToPage("/Dashboard", "ALL", "bookings", "upcoming")
                }
              >
                Upcoming{" "}
                <span className="v2-badge blue">{stats.schedule.upcoming}</span>
              </div>
            </div>
          )}
          ...
          <div className="sidebar-v2-footer">
            <div
              ...
              onClick={() => goToPage("/manage-events")}
            >
              <FaBullhorn className="v2-icon" /> Manage Events
            </div>
            <div
              className="v2-nav-item"
              onClick={() => goToPage("/priest-gallery")}
            >
              <FaRegImages className="v2-icon" /> Gallery
            </div>
            <div className="v2-nav-item" onClick={() => goToPage("/Help")}>
              <MdOutlineHelpOutline className="v2-icon" /> Help
            </div>
            <div
              className="v2-nav-item logout"
              onClick={() => setShowLogoutConfirm(true)}
            >
              <MdOutlineLogout className="v2-icon" /> Logout
            </div>
          </div>
        </div>
      </aside>
