...
export default function TeluguCalendar() {
  ...
  const monthlyFestivals = useMemo(() => {
    return festivals
      .filter((f) => {
        ...
        return (
          getMonth(festivalDate) === getMonth(activeStartDate) &&
          getYear(festivalDate) === getYear(activeStartDate)
        );
      })
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(
        `${API_BASE}/api/panchangam/month?year=${year}&month=${month}`,
      ).then((r) => (r.ok ? r.json() : [])),
      fetch(`${API_BASE}/api/festivals/month?year=${year}&month=${month}`).then(
        (r) => (r.ok ? r.json() : []),
      ),
    ]).then(([panch, fest]) => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API_BASE}/api/daily-times/by-date/${dateStr}`).then((r) =>
        r.ok ? r.json() : null,
      ),
      fetch(`${API_BASE}/api/sun?date=${dateStr}`).then((r) =>
        r.ok ? r.json() : null,
      ),
    ]).then(([daily, sun]) => {
      setDailyTimes(daily);
  ...
  const tileContent = ({ date, view }) => {
    ...
    let moonSymbol =
      panch?.tithi === "Pournami"
        ? "🌕"
        : panch?.tithi === "Amavasya"
          ? "🌑"
          : null;
      ...
      <div className="calendar-grid-layout">
        ...
        <aside className="calendar-sidebar">
          <div className="sidebar-card fest-card">
            <h3>
              <FaRegCalendarCheck /> {format(activeStartDate, "MMMM")} Festivals
            </h3>
            <div className="sidebar-scroll">
              {monthlyFestivals.length > 0 ? (
                monthlyFestivals.map((f, i) => (
          ...
          <div className="sidebar-card appt-card">
            <h3>
              <FaRegListAlt /> Rituals: {format(selectedDate, "MMM d")}
            </h3>
            <div className="sidebar-scroll">
              {(
        ...
        <main className="calendar-center">
          ...
          <section className="day-details-premium">
            ...
            {isLoadingDetails ? (
              <div className="loader-ring">Processing...</div>
            ) : (
              <div className="timings-grid-modern">
                ...
                <div className="t-card rahu">
                  ...
                  <p>
                    {dailyTimes
                      ? `${formatTime12Hour(dailyTimes.rahukalamStart)} - ${formatTime12Hour(dailyTimes.rahukalamEnd)}`
                      : "N/A"}
                  </p>
                </div>
                <div className="t-card yama">
                  ...
                  <p>
                    {dailyTimes
                      ? `${formatTime12Hour(dailyTimes.yamagandamStart)} - ${formatTime12Hour(dailyTimes.yamagandamEnd)}`
                      : "N/A"}
                  </p>
                </div>
              </div>
