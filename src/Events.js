...
import { 
    ...
    FaCalendarAlt, FaSun, FaMoon, FaOm, FaClock, 
    FaDharmachakra, FaArrowRight, FaStar 
} from 'react-icons/fa';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/sun?date=${todayStr}`).then(res => res.json()).then(sun => {
      fetch(`${API_BASE}/api/daily-times/by-date=${todayStr}`).then(res => res.json()).then(daily => {
        setTodayTimings({ ...sun, ...daily });
        setIsTimingsLoading(false);
      });
    }).catch(() => setIsTimingsLoading(false));

    fetch(`${API_BASE}/api/gallery/all-random`).then(res => res.json()).then(data => {
        const normalized = data.map(img => ({ imageUrl: `${API_BASE}${img.imageUrl}`, title: img.caption }));
        setCarouselImages(normalized.length > 0 ? normalized : [{ imageUrl: 'https://images.unsplash.com/photo-1604318721473-cb7223b37803?q=80&w=2070', title: 'Sacred Tradition' }]);
    });
  }, []);
    ...
    <div className="db-page-wrapper">
      <section className="db-hero">
        <div className="db-hero-content">
          ...
          <p>Your gateway to traditional Vedic services and auspicious timings.</p>
        </div>
      </section>
        ...
        <div className="db-grid-layout">
          ...
          <main className="db-column">
            <div className="db-tile-row">
                <div className="db-tile db-saffron" onClick={() => navigate('/book-priest')}>
                ...
                <div className="db-tile db-gold" onClick={() => navigate('/pooja-items')}>
                ...
                <div className="db-tile db-charcoal" onClick={() => navigate('/prasadam')}>
            ...
            <div className="db-promo-card">
               <img src={carouselImages[currentImageIndex]?.imageUrl} alt="Sacred" />
               <div className="db-promo-overlay">
                  <h4>Sacred Tradition</h4>
                  <div className="db-promo-nav">
                    <button onClick={(e) => { e.stopPropagation(); prevImage(); }}><FaChevronLeft /></button>
                    <button onClick={(e) => { e.stopPropagation(); nextImage(); }}><FaChevronRight /></button>
                  </div>
               </div>
            </div>
          </main>
               ...
               <div className="db-p-body">
                  <div className="db-sun-row">
                    <div className="db-p-unit">
                       <label>SUNRISE</label>
                       <strong>{formatTime12Hour(todayTimings?.sunrise) || '07:09 AM'}</strong>
                    </div>
                    <div className="db-p-unit">
                       <label>SUNSET</label>
                       <strong>{formatTime12Hour(todayTimings?.sunset) || '07:50 PM'}</strong>
                    </div>
                  </div>
                  
                  <div className="db-astro-row">
                    <div className="db-astro-item">
                        <label><FaStar /> NAKSHATRAM</label>
                        <p>{todayTimings?.nakshatram || "---"}</p>
                    </div>
                    <div className="db-astro-item">
                        <label><FaOm /> TITHI</label>
                  ...
                  <div className="db-kalam-box">
                    <div className="db-k-row">
                        ...
                        <p>{todayTimings?.rahukalamStart ? `${formatTime12Hour(todayTimings.rahukalamStart)} - ${formatTime12Hour(todayTimings.rahukalamEnd)}` : "--- : ---"}</p>
                    </div>
                    <div className="db-k-row">
                        <label>YAMAGANDAM</label>
                        <p>{todayTimings?.yamagandamStart ? `${formatTime12Hour(todayTimings.yamagandamStart)} - ${formatTime12Hour(todayTimings.yamagandamEnd)}` : "--- : ---"}</p>
                    </div>
                  </div>
               </div>
            </div>
