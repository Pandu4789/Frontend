    ...
    const DayTimeSlots = ({ onAvailabilitySave }) => {
        ...
        return (
            <div className={`am-timeslot-panel ${isPast ? 'disabled' : ''}`}>
                ...
                {draftSlots && ( <div className="am-save-actions"> <p>You have unsaved changes.</p> <div> <button className="am-cancel-btn" onClick={() => setDraftSlots(null)}>Cancel</button> <button className="am-save-btn" onClick={handleSaveChanges}>Save Changes</button> </div> </div> )}
