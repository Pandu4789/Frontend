...
import { 
    ...
    isSameDay, parse, isBefore, startOfDay, parseISO 
} from 'date-fns';
import { 
...
const AvailabilityManager = ({ activeView }) => {
    ...
    const CalendarHeader = () => (
        <div className="am-calendar-header">
            ...
            <button onClick={() => setCurrentDate(addMonths(currentDate, 1))}><FaChevronRight /></button>
        </div>
    );
        ...
        return (
            <div className="am-calendar-grid">
                {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(d => (
                    <div className="am-day-name" key={d}>{d}</div>
                ))}
                {days.map(day => {
                    const isPast = isBefore(day, today);
    ...
    const DayTimeSlots = ({ onSave }) => {
        ...
        const handleSlotClick = (time, status) => {
            ...
            const nextStatus = current[time] === 'Available' ? 'Unavailable' : 'Available';
        ...
        const handleSaveChanges = async () => {
            ...
            try {
                await axios.post(`${API_BASE}/api/availability`, { priestId, date: dateStr, unavailableSlots: unavailable });
        ...
        const applyTemplate = () => {
            ...
            Object.keys(newSlots).forEach(t => {
                ...
                if (newSlots[t] !== 'Booked') {
                    newSlots[t] = (h >= sH && h <= eH && !template.breaks.includes(t)) ? 'Available' : 'Unavailable';
                }
            });
            ...
            <div className={`am-timeslot-panel ${isShaking ? 'error-shaking' : ''}`}>
                {overrideSlot && (
                    <ConfirmationModal 
                        ...
                        onConfirm={() => {
                            setDraftSlots({ ...(draftSlots || originalSlots), [overrideSlot]: 'Available' });
                ...
                <button className="am-edit-template-btn" onClick={() => setShowSettings(!showSettings)}>
                ...
                {showSettings && (
                    <div className="template-settings">
                        <div className="time-select-group">
                            ...
                            <select value={template.start} onChange={e => setTemplate({...template, start: e.target.value})}>
                                {allPossibleSlots.map(t => <option key={t} value={t}>{format(parse(t, 'HH:mm', new Date()), 'h:mm a')}</option>)}
                            </select>
                        </div>
                        <div className="time-select-group">
                            <label>End of Day</label>
                            <select value={template.end} onChange={e => setTemplate({...template, end: e.target.value})}>
                                {allPossibleSlots.map(t => <option key={t} value={t}>{format(parse(t, 'HH:mm', new Date()), 'h:mm a')}</option>)}
                            </select>
                        </div>
                        <div className="break-settings">
                            ...
                            <div className="break-slots-grid">
                                {allPossibleSlots.filter(t => {
                                    const h = parseInt(t.split(':')[0]);
                                    return h >= parseInt(template.start.split(':')[0]) && h <= parseInt(template.end.split(':')[0]);
                                }).map(t => (
                                    <button 
                                        key={t} 
                                        className={`break-slot-btn ${template.breaks.includes(t) ? 'is-break' : ''}`}
                                        onClick={() => setTemplate({...template, breaks: template.breaks.includes(t) ? template.breaks.filter(b => b !== t) : [...template.breaks, t]})}
                                    >
                        ...
                        <button className="am-apply-template-btn" onClick={applyTemplate}>Apply Template</button>
                    </div>
                )}

                <div className="am-panel-header-info">
                    <h3 className="am-panel-subtitle">Availability for</h3>
                    <h2 className="am-panel-title-date">{format(selectedDate, 'EEEE, MMM d')}</h2>
                </div>

                {isPast && <div className="am-past-day-notice">Past dates cannot be modified.</div>}

                {draftSlots && (
                    <div className="am-save-actions">
                        <span>Unsaved Changes</span>
                        <div className="am-action-btns">
                            <button className="am-cancel-btn" onClick={() => setDraftSlots(null)}>Discard</button>
                            <button className="am-save-btn" onClick={handleSaveChanges}>Save Changes</button>
                        </div>
                    </div>
                )}
