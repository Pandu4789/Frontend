...
const PriestDashboard = () => {
  ...
  const [userName, setUserName] = useState(
    localStorage.getItem("firstName") || "Priest",
  );
  ...
  const fetchData = async () => {
    ...
    try {
      const [
        ...
        // --- UPDATED: DYNAMIC PARAMETERS FOR PANCHANG ---
        fetch(
  ...
  const todayBookings = useMemo(() => {
    return allAppointments.filter((b) => {
      ...
      return (
        isToday(parseISO(cleanDate)) && b.status?.toUpperCase() !== "REJECTED"
      );
    });
  }, [allAppointments]);
  ...
  const upcomingBookings = useMemo(
    () =>
      allAppointments.filter(
        (b) =>
          b.date &&
          isAfter(parseISO(b.date), startOfDay(new Date())) &&
  ...
  const pendingRequests = useMemo(() => {
    const pBookings = allAppointments
      .filter((b) => b.status?.toUpperCase() === "PENDING")
      .map((b) => ({ ...b, type: "BOOKING" }));
    const pMuhurtams = muhurtamRequests
      .filter((r) => !r.viewed)
      .map((r) => ({ ...r, type: "MUHURTAM" }));
  ...
  const handleAction = async (actionType) => {
    ...
    try {
      if (selectedRequest.type === "MUHURTAM") {
        await axios.put(
          `${API_BASE}/api/muhurtam/${selectedRequest.id}/viewed`,
          { viewed: true },
        );
        ...
        await axios.put(
          `${API_BASE}/api/booking/${endpoint}/${selectedRequest.id}`,
        );
        actionType === "accept"
  ...
  const ActionCard = ({ item, isPending }) => {
    const isExpired =
      item.type === "BOOKING" &&
      item.date &&
      isBefore(parseISO(item.date), startOfDay(new Date()));

    return (
      <div
        className={`db-request-card ${item.type === "MUHURTAM" ? "muh-card" : "app-card"} ${isExpired ? "expired-gray" : ""}`}
      >
        <div className="req-header">
          <span className="req-badge">
            {item.type === "MUHURTAM" ? "Muhurtam Inquiry" : "Booking Request"}
          </span>
          {isExpired && (
            <span className="status-pill status-expired">Expired</span>
          )}
          {!isPending && !isExpired && (
  ...
  if (isLoading)
    return (
      <div className="priest-loader">Synchronizing Divine Workspace...</div>
    );
    ...
    <div className="db-page-wrapper">
        ...
        <div className="db-grid-layout">
          <aside className="db-column db-sticky-sidebar">
            <div className="db-content-card">
              <div className="db-card-head">
                <FaChartBar /> <h2>Performance</h2>
              </div>
              <div className="priest-mini-stats">
                <div
                  className={`m-stat ${activeView === "today" ? "m-active" : ""}`}
                ...
                <div
                  className={`m-stat ${activeView === "pending" ? "m-active" : ""}`}
                  onClick={() => handleTabChange("pending")}
                >
                  <label>Requests</label>
                  <strong className="txt-orange">
          ...
          <main className="db-column center-col">
            <div className="db-tile-row">
              ...
              <div
                className={`db-tile db-charcoal ${activeView === "stats" ? "tile-active" : ""}`}
            ...
            <div className="priest-view-container" ref={contentRef}>
              ...
              <div
                className={`view-content-wrapper ${["today", "upcoming", "pending"].includes(activeView) ? "grid-mode" : "full-page-mode"}`}
              >
                {activeView === "today" &&
                  (todayBookings.length > 0 ? (
                    todayBookings.map((item) => (
                      <ActionCard key={item.id} item={item} isPending={false} />
                    ))
                  ) : (
                    <div className="empty-state">
                      No ritual tasks for today.
                    </div>
                  ))}
                {activeView === "upcoming" &&
                ...
                {activeView === "stats" && (
                  <div className="full-width-view">
                    <PoojaStatsPage stats={stats} />
                  </div>
                )}
                {activeView === "events" && (
                  <div className="full-width-view">
                    <ManageEventsPage events={templeEvents} />
                  </div>
                )}
                {activeView === "knowledge" && (
                  <div className="full-width-view rich-editor-container">
                    <div className="editor-header-hint">
                      <FaEdit /> Digital Mantra Bank
                    </div>
                    <KnowledgeBase
          ...
          <aside className="db-column db-sticky-sidebar">
            ...
            <div className="db-content-card action-list">
              <div className="db-card-head">
                <FaDharmachakra /> <h2>Shortcuts</h2>
              </div>
              <button
                className={`s-link ${activeView === "upcoming" ? "s-active" : ""}`}
                onClick={() => handleTabChange("upcoming")}
              >
                <FaRegCalendarAlt /> Upcoming Bookings
              </button>
              <button
                className={`s-link ${activeView === "events" ? "s-active" : ""}`}
                onClick={() => handleTabChange("events")}
              >
                <FaBullhorn /> Manage Events
              </button>
              <button
                className={`s-link ${activeView === "knowledge" ? "s-active" : ""}`}
                onClick={() => handleTabChange("knowledge")}
              >
                <FaStickyNote /> Mantra Notes
              </button>
            </div>
          </aside>
        </div>
            ...
            <div className="req-overlay-body">
              ...
              <div className="ov-section">
                <div className="ov-item">
                  <FaUserTie /> <span>{selectedRequest.customerName}</span>
                </div>
                <div className="ov-item">
                  <FaPhoneAlt /> <span>{selectedRequest.contact || "N/A"}</span>
                </div>
                <div className="ov-item">
                  <FaEnvelope /> <span>{selectedRequest.email || "N/A"}</span>
                </div>
              </div>
                ...
                {selectedRequest.type === "MUHURTAM" ? (
                  <>
                    <div className="ov-detail-row">
                      ...
                      <strong>{selectedRequest.nakshatram || "---"}</strong>
                    </div>
                    <div className="ov-detail-row">
                    ...
                    <div className="ov-detail-row">
                      <label>Place</label>{" "}
                      <strong>{selectedRequest.place || "---"}</strong>
                    </div>
                  </>
                ) : (
                  <>
      ...
      {isModalOpen && (
        <AppointmentModal
          ...
          onSave={() => fetchData()}
          priest={{ id: priestId, firstName: userName }}
        />
      )}
    </div>
  );
