...
import {
    FaCalendarCheck, FaCalendarAlt, FaBell, FaDownload, FaPlus, FaStickyNote, FaBullhorn,
    FaChartBar, FaUserClock, FaRegCalendarAlt, FaListAlt, FaChevronRight, FaCalendarDay
} from 'react-icons/fa';
import './Dashboard.css';

// --- View for Displaying Booking/Request Cards (No JS changes needed) ---
const BookingsListView = ({ title, items, type = 'booking' }) => {
    if (!items || items.length === 0) {
        return <div className="pd-no-data">No {type === 'booking' ? 'bookings' : 'requests'} for this view.</div>;
    }
    return (
        <div>
                ...
                {items.map(item => (
                    <div key={item.id} className="pd-card booking-card">
                        <div className="pd-card-header">
                            ...
                            {type === 'booking' && <span className={`pd-status-badge status-${item.status.toLowerCase()}`}>{item.status}</span>}
                        </div>
                        <div className="pd-card-body">
                            <p><strong>Customer:</strong> {item.customerName} {item.contact ? `(${item.contact})` : ''}</p>
                            <p><strong>Date:</strong> {format(parseISO(item.date), 'EEE, MMM d, yyyy')}</p>
...
// --- Other Views (No JS changes needed) ---
...
// --- REBUILT: View for Managing Temple Events ---
        ...
        <div>
            ...
            {showAddForm && (
                <form onSubmit={handleFormSubmit} className="add-event-form">
                    <input type="text" name="title" placeholder="Event Title" value={newEvent.title} onChange={handleFormChange} required />
                    ...
                    <textarea name="description" placeholder="Brief description..." value={newEvent.description} onChange={handleFormChange} required />
                    ...
                    <div key={event.id} className="pd-card event-manage-card">
                        ...
                        <div className="event-details">
                            <div className="event-details-header">
                                ...
                                <div className="pd-card-actions">
                                    <button className="pd-action-btn secondary small">Edit</button>
                                    <button className="pd-action-btn danger">Cancel</button>
                                </div>
                            </div>
...
const PriestDashboard = () => {
    ...
    useEffect(() => {
        ...
        setPendingRequests(mockData.pendingRequests);
        setStats(mockData.stats);
        setTransactions(mockData.transactions);
        setTempleEvents(mockData.templeEvents);
    }, []);
    ...
    const renderActiveView = () => {
        switch (activeView) {
            ...
            case 'pending': return <BookingsListView title="Pending Booking Requests" items={pendingRequests} type="request" />;
            case 'stats': return <PoojaStatsView stats={stats} transactions={transactions} onExport={downloadFinancialsCSV} />;
    ...
    return (
        <div className="priest-dashboard-container">
            ...
            <section className="pd-dashboard-section cta-section"><h2 className="pd-section-title">Manage Your Schedule</h2><p>Control your availability to receive new bookings or add appointments manually.</p><div className="cta-buttons"><button className="cta-button primary" onClick={() => navigate('/availability-manager')}><FaCalendarDay /> Manage Availability <FaChevronRight className="cta-button-icon" /></button><button className="cta-button secondary" onClick={() => navigate ('/appointmentmodal')}><FaPlus /> Add Manual Booking</button></div></section>
...
const getMockData = () => {
    ...
    return {
        bookings: [ { id: 1, date: todayStr, poojaType: 'Ganesh Pooja', customerName: 'Rohan Sharma', contact: '123-456-7890', startTime: '09:00 AM', status: 'Upcoming' }, { id: 2, date: todayStr, poojaType: 'Satyanarayan Puja', customerName: 'Priya Patel', contact: '234-567-8901', startTime: '05:00 PM', status: 'In Progress' }, { id: 3, date: tomorrowStr, poojaType: 'Griha Pravesh', customerName: 'Amit Singh', contact: '345-678-9012', startTime: '11:00 AM', status: 'Upcoming' }],
        pendingRequests: [ { id: 201, date: '2025-06-25T10:00:00.000Z', poojaType: 'Vastu Shanti', customerName: 'Neha Desai' }, { id: 202, date: '2025-06-28T10:00:00.000Z', poojaType: 'Birthday Havan', customerName: 'Vikram Mehta' } ],
        stats: { week: 5, month: 22, year: 150, topPoojas: [{ name: 'Satyanarayan Puja', count: 45 }, { name: 'Ganesh Pooja', count: 30 }, { name: 'Vastu Shanti', count: 20 }] },
        transactions: [ { id: 301, date: '2025-06-02T10:00:00.000Z', customerName: 'Rohan Sharma', poojaType: 'Vastu Shanti', amount: 251.00, status: 'Paid' }, { id: 302, date: '2025-06-05T10:00:00.000Z', customerName: 'Priya Patel', poojaType: 'Navagraha Pooja', amount: 201.00, status: 'Paid' }, { id: 303, date: '2025-06-08T10:00:00.000Z', customerName: 'Amit Singh', poojaType: 'Car Pooja', amount: 151.00, status: 'Pending' } ],
        templeEvents: [
            { id: 10, title: 'Ganesh Chaturthi Celebration', date: '2025-08-29T18:00:00.000Z', location: 'Main Prayer Hall', description: 'Join us for special aarti and prasad distribution.' },
            { id: 11, title: 'Diwali Mela', date: '2025-10-21T11:00:00.000Z', location: 'Temple Grounds', description: 'A festive fair with stalls, food, and cultural programs.' },
        ]
    };
};